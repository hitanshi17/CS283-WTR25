...
int get_student(int fd, int id, student_t *s)
{
    ...
    off_t offset = (off_t)id * sizeof(student_t);
    ...
    student_t empty = EMPTY_STUDENT_RECORD; // {0}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    student_t old_records;
    int rc = get_student(fd, id, &old_records);
    if (rc == NO_ERROR) {
    ...
    new_student.lname[sizeof(new_student.lname)-1] = '\0'; // Ensure null-termination
    new_student.gpa = gpa;
...
int count_db_records(int fd)
{
    ...
    ssize_t bytes;
    while ((bytes = read(fd, &student, sizeof(student_t))) == sizeof(student_t)) {
...
int compress_db(int fd) {
    ...
    size_t record_size = sizeof(student_t);
    student_t student, empty = EMPTY_STUDENT_RECORD;
    ssize_t bytes_read;
    bool records_moved = false;  

    while ((bytes_read = read(fd, &student, record_size)) == (ssize_t)record_size) {
        ...
        if (write(tmp_fd, &student, record_size) != (ssize_t)record_size) {
            printf(M_ERR_DB_WRITE);
            close(tmp_fd);
            return ERR_DB_FILE;
        }
    ...
    if (!records_moved) {
        printf("Database compression not needed; no empty records.\n");
